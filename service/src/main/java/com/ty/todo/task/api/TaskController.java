...
public class TaskController {
    ...
    public ResponseEntity<TaskResponse> create(@Valid @RequestBody TaskRequest request) {
        ...
        TaskResponse body = new TaskResponse(created.id(), created.title(), created.remindAt(), created.email(), created.remindedAt());
    ...
    public List<TaskResponse> list() {
        return service.listTasks().stream()
                .map(task -> new TaskResponse(task.id(), task.title(), task.remindAt(), task.email(), task.remindedAt()))
                .toList();
    }
}
